...
void lcd_goto_xy(uint8_t x, uint8_t y)
{
	...
	switch(y)
	{
		...
		case 0: 
				x_pos = x;
				y_pos = 0;
				RAMAddr = LCD_LINE0_RAMADDR + x; 
				break;
		case 1: 
				x_pos = x;
				y_pos = 1;
				RAMAddr = LCD_LINE1_RAMADDR + x;
				break;
		case 2: 
				x_pos = x;
				y_pos = 2;
				RAMAddr = LCD_LINE2_RAMADDR + x;
				break;
		case 3: 
				x_pos = x;
				y_pos = 3;
				RAMAddr = LCD_LINE3_RAMADDR + x;
				break;
	}
	// set data address
...
//=================================================================
// Put a character on LCD at x,y position.
//=================================================================
...
void lcd_put_int_with_width(int x, int width)
{
	...
	sprintf(temp, "%*d", width, x); 
	lcd_puts(temp);
}

void lcd_perform_backspace(void)
{
	lcd_sendCommand(0x10); // Set the cursor to move left
	lcd_putc(' ');
...
void lcd_clear_line(int y){
	lcd_clear_selected(0, DSP_WIDTH, y);
}
